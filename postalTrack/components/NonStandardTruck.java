 ...
 * <br>This class represents A vehicle for transporting packages of non-standard size .
 * <br>extend from @Truck
...
public class NonStandardTruck extends Truck{
	 ...
	 * <br>Available vehicle does nothing.
	 * <br>A vehicle that is in the middle of a trip reduces the time left to the end of the trip (timeLeft) by 1.
	 * <br>If after the reduction the time value is equal to zero, then the trip is over and a vehicle has reached its destination.
	 *  If the purpose of the trip was to collect a package from a sending customer (COLLECTION), the vehicle will be transferred for delivery.
	 *  <br>If the purpose of the trip was to deliver the package to the customer (DISTRIBUTION), 
			...
			if (!this.isAvailable()) {
				...
				if (this.getTimeLeft()==0) {
					if (p.getStatus()==Status.COLLECTION) {
						...
						p.addTracking(new Tracking(MainOffice.getClock(), null, p.getStatus()),p);
						setAvailable(true);
					}
				}
			}
